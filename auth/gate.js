...
(function () {
    ...
    /* Everything below the sign-in helpers is declaration only until
     * `var existing = readSession()`. This block has to stay above that
     * line: for a returning visitor the IIFE takes the early return a few
     * lines down, so any `var` declared after it is hoisted but never
     * assigned. That shipped once — the transport config lived further
     * down the file, every signed-in page hit
     * "Cannot read properties of undefined (reading 'push')" on its
     * first request, and the admin dashboard reported it as the backend
     * refusing to answer. Keep new transport state here, not beside
     * whatever uses it. */
    /* ---------- Request transport ----------
     *
